...
while true; do
  ...
  if ! python3 create_commute_audio.py >> commutes/logs/commute-audio.log 2>&1; then
    osascript -e 'display notification "Commute audio creation hit an error. Check commutes/logs/commute-audio.log." with title "Commute automation failed"' >/dev/null 2>&1 || true
  fi
  if ! python3 commute_jobs.py >> commutes/logs/commute-watch.log 2>&1; then
    osascript -e 'display notification "Commute audio handoff hit an error. Check commutes/logs/commute-watch.log." with title "Commute automation failed"' >/dev/null 2>&1 || true
  fi
  sleep 600
done
